...
const answer = (value: string | undefined, revealed: boolean) => revealed ? <span className="revealed-form">{value || "—"}</span> : <span className="chart-blank" aria-label="blank answer" />;
...
function Matrix({ items, revealed, kind }: { items: HenleSourceCard[]; revealed: boolean; kind: "case" | "person" }) {
  ...
  return <div className="chart-scroll"><table className="henle-chart"><thead><tr><th>{kind === "case" ? "Case" : "Person"}</th>{cols.map((col) => <th key={col}>{col}</th>)}</tr></thead><tbody>{labels.map((label) => <tr key={label}><th>{label}</th>{cols.map((col) => <td key={col}>{answer(rows.find((row) => row.row === label && row.number === col)?.item.answer, revealed)}</td>)}</tr>)}</tbody></table></div>;
}
...
function PrincipalParts({ items, revealed }: { items: HenleSourceCard[]; revealed: boolean }) {
  ...
  return <div className="chart-scroll"><table className="henle-chart"><thead><tr><th>Conjugation</th>{parts.map((part) => <th key={part}>{part}</th>)}</tr></thead><tbody>{conjugations.map((conjugation) => <tr key={conjugation}><th>{conjugation}</th>{parts.map((part) => <td key={part}>{answer(rows.find((row) => row.conjugation === conjugation && row.part === part)?.item.answer, revealed)}</td>)}</tr>)}</tbody></table></div>;
}

function Generic({ items, revealed }: { items: HenleSourceCard[]; revealed: boolean }) { return <div className="chart-scroll"><table className="henle-chart"><thead><tr><th>Prompt</th><th>Form</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><th>{item.prompt}</th><td>{answer(item.answer, revealed)}</td></tr>)}</tbody></table></div>; }
